...
An issue I was not able to resolve was coding the two next level maps of the game. I attempted to do the transitions between rooms within my branch (Han-File), however it would not work when opening up onto the live server. I had tried looking at the console and the example on gitHub but it came up with the same issue - the lexical declaration 'currentLevel' cannot be accessed before initialisation. I was unsure what to do to fix this in my code. 
...
Due to the game not having multiple maps, space was limited so there is only two enemies included in the game (we intended to have different enemies per stage) however the code for the king enemy can be found within my branch (Imogen-File). I had also illustrated the king which we were intending to have as the final boss on the final level, but we did not manage to get to the stage of coding the other levels. 
...
I struggled quite alot when it came to allowing my VS Code to work on my pc so I'm incredibly grateful for Anna helping me out when I really panicked. I also had some music made by one of my friends just to add to the story; helping him out with his own project too. I really have learnt so much about coding and structuring a game design; as I've really been inspired and pushed by the rest of the group being influences and inspired by their efforts. I really enjoyed this project.
